...
#include <wls/manifest/plugins.hpp>
...
int main(int argc, char** argv) {
   wls::plugin::initialize_plugin_factories();
   ...
   try {
      ...
      std::cerr << "------------------------------------------------------\n";
      auto initminer_private_key = graphene::utilities::key_to_wif( WLS_INIT_PRIVATE_KEY );
      ...
      for( const std::string& plugin_name : wls::plugin::get_available_plugins() )
         node->register_abstract_plugin( wls::plugin::create_plugin( plugin_name, node ) );
      ...
      {
         std::cout << "whaleshares_blockchain_version: " << fc::string( WLS_BLOCKCHAIN_VERSION ) << "\n";
         std::cout << "whaleshares_git_revision:       " << fc::string( graphene::utilities::git_revision_sha ) << "\n";
